module Decor_Standards
	...
	def self.load_dynamic_config
		...
		if @selection.length != 0 and @selection.length == 1
			dialog = UI::WebDialog.new("Decorpot - Dynamic Configure", true, "Decorpot - Dynamic Configure", 300, 1000, 10, 150, true)
			...
			dialog.add_action_callback("loadingpage") {|key, val|
				...
				lamn_code =get_lamination seln
				if lamn_code
					loadval << "Lamination Code/"+lamn_code
				end
				seln.definition.attribute_dictionaries["dynamic_attributes"].map{|dynamique_attibute,dynamique_value|
				...
				puts "loadval : #{loadval} : #{title} : #{proname}"
